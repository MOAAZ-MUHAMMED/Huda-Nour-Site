...
export function DoorIntro({ onComplete }: DoorIntroProps) {
  ...
  useEffect(() => {
    // Hold the door closed for a moment, then start opening
    const openTimer = setTimeout(() => {
      setIsOpen(true);
    }, 1500);

    // Remove from DOM after transition completes (animation duration is 2.2s now for more majestic feel)
  ...
  return (
    <div 
      className={`fixed inset-0 z-[99999] flex overflow-hidden bg-transparent transition-opacity duration-1000 ${
    ...
    >
      ...
      {/* Middle Vertical Glow Line (splits or fades out as doors open) */}
      <div 
